...
4. おすすめ提案更新（proposals.json）
   ...
   - `url` は可能な限りその公演固有のページ（劇団公式の公演詳細、CoRich/ライブポケット等のチケットページ）にする。会場の月間ラインナップ一覧ページなど「そこから該当公演の詳細に辿り着けないURL」は避ける（劇団道学先生の提案でこの問題が指摘されたことがある）。個別ページが見つからない場合はreasonに「詳細ページ未確認、劇団名で検索を」等と明記する
