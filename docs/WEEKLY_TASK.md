...
4. おすすめ提案更新（proposals.json）
   ...
   - `nextMonth`: 今日から約1ヶ月で「カレンダー未登録」の公演を2〜5件。情報収集は次の2系統を必ず両方行う（「たまたま検索でヒットした所だけ」にしない）:
     1. **会場ローテーション（新規発見・どこを探すかの基準）**: `rotation_pick.py` が出した今週の会場3件について、その会場の直近ラインナップを確認し、未登録かつ嗜好傾向（アングラ・小劇場・テント芝居・舞踏）に近い公演があれば候補にする。**会場は探索範囲を決めるための基準であって、「頻出会場だから」自体は推薦理由にならない**（会場は劇団と違って本人の嗜好と直結しない）。reasonには会場ではなく作品の中身・作風で理由を書く
     2. **劇団ローテーション（漏れ検知）**: `rotation_pick.py` が出した今週の劇団4件について、公式サイト/CoRich/X等で次回公演の有無を確認する。**これは新規開拓ではない** — 頻出劇団＝既にこのカレンダーで繰り返し登録している「好きな劇団」なので、その劇団の新作アナウンスをこちらが見落としていないかを確認するのが目的。「※登録済みあり」の劇団は今後の公演が既にカレンダーにあるので軽く確認するだけでよい（変更・追加公演がないか程度）。印なしの劇団は次回公演が見つかっていないので重点的に確認し、見つかれば候補に追加する（この場合はreasonに「頻出劇団◯◯の新作」と書いてよい。こちらは本人の嗜好と直結する）
     - 上記2系統だけで候補が2件に満たない場合に限り、それ以外のWeb検索で補ってよい
